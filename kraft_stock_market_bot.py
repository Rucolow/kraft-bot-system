...
"""
            ...
            # ランダムなイベント型を選択
            event_types = [
                "新商品発表", "技術革新", "企業買収", "提携発表", "研究開発", 
                "事故・問題", "規制変更", "CEO交代", "工場建設", "市場参入",
                "特許取得", "業績上方修正", "業績下方修正", "設備投資", "環境対応"
            ]
            
            event_type = random.choice(event_types)
            
            # より劇的で面白いニュースプロンプト
            prompt = f"""KRAFT株式市場で起こった刺激的な企業ニュースを1つ生成してください。
...
- 「💥 ソミーの新ゲーム機開発プロジェクトで重大な技術的問題が発覚、発売延期の可能性」
- 「🔥 トミタが電気自動車向け新型バッテリーを発表、充電時間を90%短縮する画期的技術」

現在の{selected_stock['name']}の状況を考慮して、{event_type}関連の劇的なニュースを生成してください。"""
        ...
        print(f"ニュース生成エラー: {e}")
        # エラー時のフォールバック - 劇的なニュース
        fallback_news = [
            "🚨 KRAFT市場で謎の大口取引が発生！機関投資家の動きに注目が集まる",
            "💥 主要銘柄で同時多発的な価格変動！市場関係者は『異常事態』と警戒",
            "🔥 複数の企業が同日に重大発表を予告、明日の市場開始が待たれる",
            "⚡ 新興企業の技術が既存大手を脅かす存在に！業界再編の可能性",
